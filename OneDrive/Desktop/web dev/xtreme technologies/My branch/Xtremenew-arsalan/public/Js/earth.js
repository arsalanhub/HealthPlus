...
const CalculateFun = () => {
  ...
  document.getElementById("slider-2").value = Math.min(totaltimetaken, 240);
...
window.addEventListener("hashchange", () => {
  ...
  let sz = url.length;
  if (url[sz - 1] == "equirectangular") {
    window.location.href += "=-277.50,0.00,261";
...
$("#weather_picker_btn").click(function () {
  $("button.show-menu").click();
});
...
var currentFile = "",
  filename = "",
  currentVisualization = false,
  windObject = {}, // extracted data (readable data) from gzip wind file
  waveObject = {},
  waveGzipObject = {},
  hours_to_be_added = "000",
  data_day,
  data_slot,
  dataGeneratorHandler = true;
wind404set = new Set();
wave404set = new Set();

function fileNameGenerator(month, day, slot) {
...
function windDataGenerator(prefilename) {
  if (windObject[prefilename] !== undefined || wind404set.has(prefilename)) {
    return false;
  }
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    xhr.open(
      "GET",
      `./data/weather/wind/${m}_${data_day}_${data_slot}_${hours_to_be_added}_wind.gz`,
      true
    );
    xhr.onload = function () {
      if (xhr.status == 200) {
  ...
  promise.then(function (xhr) {
    ...
    let windFile = pako.ungzip(byteArray, { to: "string" });
    windFile = JSON.parse(windFile);

    windObject[prefilename] = windFile;
  });

  return true;
}

function waveDataGenerator(prefilename) {
  if (
    waveObject[prefilename] !== undefined ||
    waveGzipObject[prefilename] !== undefined ||
    wave404set.has(prefilename)
  ) {
    return false;
  }
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    xhr.open(
      "GET",
      `./data/weather/wave/${m}_${data_day}_${data_slot}_${hours_to_be_added}_wave.gz`,
      true
    );
    xhr.onload = function () {
      if (xhr.status == 200) {
  ...
  promise.then(function (xhr) {
    waveGzipObject[prefilename] = xhr;
  });
...
function dataGenerator() {
  let newDate = new Date(utc + 3600000 * "-4.0");
  let prefilename = ""; // filename for data generator
  let number_of_file = 2;

  for (let i = 0; i < 20 && number_of_file > 0; i++) {
    let month = (newDate.getMonth() + 1).toString();
    let nextd = newDate.getDate().toString();
    let nextt = newDate.getHours().toString();
    let nextslot = slotGenerator(nextt);

    fileNameGenerator(month, nextd, nextslot);

    prefilename = `${month}_${data_day}_${data_slot}_${hours_to_be_added}`;

    if (Number(hours_to_be_added) > 90) break;

    if (windDataGenerator(prefilename)) {
      number_of_file--;
    }

    if (waveDataGenerator(prefilename)) {
      number_of_file--;
    }

    if (newDate.getHours() + 6 > 24) {
      newDate.setDate(newDate.getDate() + 1);
      newDate.setHours(newDate.getHours() + 6 - 24);
    } else {
...
// setInterval(() => {
//   if(dataGeneratorHandler && Object.keys(waveDataGenerator).length < 10){
//     dataGenerator();
//   }
// }, 6000);
...
var products = (function () {
  ...
  function gfs1p0degPath(type, year, month, day, slot) {
    ...
    let Download = document.getElementById("downloadCSV");
    Upload.disabled = true;
    Download.disabled = true;
    Upload.classList.add("disableUploadDownload");
    Download.classList.add("disableUploadDownload");

    dataGeneratorHandler = false;

    console.log("gfs called");

    fileNameGenerator(month, day, slot);

    filename = `${month}_${data_day}_${data_slot}_${hours_to_be_added}`;

    console.log(filename);

    if (type === "wind") {
      let found = false;
      if (windObject[filename] !== undefined) {
        found = true;
      }
      ...
      if (!found) {
        ...
        xhr.open(
          "GET",
          `./data/weather/wind/${month}_${data_day}_${data_slot}_${hours_to_be_added}_wind.gz`,
          false
        );
        xhr.overrideMimeType("text/plain; charset=x-user-defined");
        xhr.send();
        ...
        if (xhr.status === 404) {
          let status = 404;
          let currFileDate = datetime;

          // while(status === 404)
          // {
          //   let nextd = currFileDate.getDate().toString();
          //   let nextt = currFileDate.getHours().toString();
          //   let nextslot = slotGenerator(nextt);
          ...
          //   xhr.open(
          //     "GET",
          //     `./wind/${month}_${data_day}_${data_slot}_${hours_to_be_added}_wind.gz`,
          //     false
          //   );
          //   xhr.overrideMimeType("text/plain; charset=x-user-defined");
          //   xhr.send();
          ...
          //   if (currFileDate.getHours() - 6 < 0) {
          //     currFileDate.setDate(currFileDate.getDate() - 1);
          //     currFileDate.setHours(24 + (currFileDate.getHours() - 6));
          //   } else {
          //     currFileDate.setHours(currFileDate.getHours() - 6);
          //   }
          // }
        }
      ...
      if (waveObject[filename] !== undefined) {
        // waveset of extracted files (readable form data)
        found = true;
      }

      if (!found && waveGzipObject[filename] !== undefined) {
        // waveGzipset of xhr response (binary data)

        let xhr = waveGzipObject[filename];
        found = true;
        ...
        let waveFile = pako.ungzip(byteArray, { to: "string" });
        waveFile = JSON.parse(waveFile);
        waveObject[filename] = waveFile;
      }
      ...
      if (!found) {
        ...
        xhr.open(
          "GET",
          `./data/weather/wave/${month}_${data_day}_${data_slot}_${hours_to_be_added}_wave.gz`,
          false
        );
        xhr.overrideMimeType("text/plain; charset=x-user-defined");
        xhr.send();
      ...
      if (currentVisualization) {
        ...
        currentFile = pako.ungzip(byteArray, { to: "string" });
  ...
  var FACTORIES = {
    ...
    /* 
            "temp": {
                matches: _.matches({param: "wave", overlayType: "temp"}),
                create: function(attr) {
                    return buildProduct({
                        field: "scalar",
                        type: "temp",
...
(function () {
  ...
  function buildInputController() {
    ...
    d3.select(".calculateVelocity").on("click", () => {
      ...
      if (checkvelocityarr(velocityarr)) {
        ...
        console.log("Else if condition of .calculateVelocity");
        // Save it!
        // totaltimetaken=0;
        // for(var i=0;i<rldis.length;i++)
        // {
        ...
        // d3.select("#modal-content-id").style("display", "none");
  ...
  function slideOne() {
    CalculateFun();
    var tmp = document.getElementById("slider-1");
    ...
    const change_date = (x) => {
      ...
      let hoverDatetime = new Date(utc + 3600000 * "-4.0");

      hoverDatetime.setDate(hoverDatetime.getDate() + num_days);

      if (hoverDatetime.getHours() + num_hours > 24) {
        hoverDatetime.setDate(hoverDatetime.getDate() + 1);
        hoverDatetime.setHours(hoverDatetime.getHours() + num_hours - 24);
      } else {
        hoverDatetime.setHours(hoverDatetime.getHours() + num_hours);
      }

      var nextd = hoverDatetime.getDate().toString();
      var nextm = (hoverDatetime.getMonth() + 1).toString();
      var nexty = hoverDatetime.getFullYear().toString();
      var nextt = hoverDatetime.getHours().toString();

      var nextslot = slotGenerator(nextt);

      if (
        current_d - nextd > 2 ||
        (current_d - nextd === 2 && nextslot - current_slot < 0) ||
        nextd - current_d > 5 ||
        (nextd - current_d === 5 && nextslot - current_slot > 0)
      ) {
        report.error("Data is not available for this date");
        return;
      } else {
        if (d !== nextd || slot !== nextslot) {
          d = nextd;
          slot = nextslot;
  ...
  for (let i = 0; i < 44; i++) {
    ...
    d3
      ...
      .node().innerHTML = `${s}<br>${dategiver(i)}`;
  }
  /////////////////////////////////////////////////////////////////
  ...
  //   //.attr("type", "range")
  //   .attr("min", ddd)
  //   .attr("max", parseInt(ddd) + parseInt(10));

  var anything = document.getElementById("slider-1");
  // console.log("\nThis is date " + anything.value + "\n");
  ...
  function buildRenderer(mesh, globe) {
    ...
    function getWindAtLocation(coord, type) {
      ...
      if (µ.isValue(wind)) {
        ...
        // console.log("[Units Toggle]", unitToggle);

        if (type === "weather-picker") {
          var unitToggle = createUnitToggle(
            ...
            units = unitToggle.value();
          // return [µ.formatVector(wind, units[2]), units.label];
        }
    ...
    var weatherpickerselected = true; // weather picker selector bool
    var waypoint = false; // way point selector bool
    var timeselected = 0;

    if (!waypoint) {
      d3.select("#weather_picker_btn").classed("resetBtn", true);
    }

    d3.select("#weather_picker_btn").on("click", () => {
      // document
      //   .getElementById("downloadUpload")
      //   .classList.toggle(
      //     "disableUploadDownload pure-button pure-button-disabled"
      //   );
      // document
      //   .getElementById("downloadCSV")
      //   .classList.toggle("pure-button pure-button-disabled");
      // document
      ...
      // document.getElementById("downloadUpload").style.opacity = "1";
      // document.getElementById("downloadUpload").style.cursor = "pointer";
      // document.getElementById("downloadCSV").style.cursor = "pointer";
      // document.getElementById("uploadBtn").style.cursor = "pointer";
      weatherpickerselected = !weatherpickerselected;
      waypoint = !waypoint;
      ...
      if (waypoint) {
        ...
        Upload.classList.remove("disableUploadDownload");
        Download.classList.remove("disableUploadDownload");
        uploadText.classList.remove("disableUploadDownload");
        downloadText.classList.remove("disableUploadDownload");

        d3.select("#myBtn").style("display", "block");
        d3.select("#dot1").remove();
        d3.select("#weather_picker_btn").style("background-color", "#000");
        timeselected = 0;

        d3.select("#new_ship").style("opacity", "1");

        for (let i = 0; i < arr.length; i++) {
          d3.select(`#div-mark-${i + 1}`).style("opacity", "1");

          if (i !== arr.length - 1) {
            d3.select(`#line-${i + 1}`).style("opacity", "1");
          }
        }
        ...
        d3.select("#new_ship").style("opacity", "0");

        for (let i = 0; i < arr.length; i++) {
          d3.select(`#div-mark-${i + 1}`).style("opacity", "0");

          if (i !== arr.length - 1) {
            d3.select(`#line-${i + 1}`).style("opacity", "0");
          }
        }
      }
    });
    ...
    function drawLocationMark(coord, point) {
      ...
      if (coord && _.isFinite(coord[0]) && _.isFinite(coord[1])) {
        ...
        var f1 = getWindAtLocation(coord, "weather-picker")[0];
        var f2 = getWindAtLocation(coord, "weather-picker")[1];

        if (weatherpickerselected) {
          // if (d3.event.defaultPrevented) return;
          timeselected += 1;
        ...
        for (let i = 0; i < oldpoints[0].length; i++) {
          ...
          if (
            ...
            clicked = i;
            d3.select(`.location-mark-${clicked + 1}`) // allow to select a way point (fill orange color)
              .style("fill", "#FFC400")
              .style("stroke", "#FFC400");
            return;
          }
        ...
        if (arr.length > 1) {
          ...
          if (coorarr.length >= 2) {
            pointsArr = coorarr[coorarr.length - 2];
          }
    ...
    const plotShip = (x, y) => {
      ...
      // c.append("circle");
      c.attr("id", "new_circle_ship_0")
        // .attr("class", "new_circle_ship blink_ship")
        .attr("r", "5")
        .attr("cx", x)
        .attr("cy", y);
      // c.append("circle");
      c.attr("id", "new_circle_ship_1")
        // .attr("class", "new_circle_ship_rings blink_ship")
        .attr("r", "5")
        .attr("cx", x)
        .attr("cy", y);
      // c.append("circle");
      c.attr("id", "new_circle_ship_2")
        // .attr("class", "new_circle_ship_rings blink_ship")
        .attr("r", "9")
        .attr("cx", x)
        .attr("cy", y);
      // c.append("circle");
      c.attr("id", "new_circle_ship_3")
        // .attr("class", "new_circle_ship_rings blink_ship")
        .attr("r", "13")
        .attr("cx", x)
        .attr("cy", y);
      // c.append("circle");
      c.attr("id", "new_circle_ship_4")
        // .attr("class", "new_circle_ship_rings blink_ship")
        .attr("r", "17")
        .attr("cx", x)
        .attr("cy", y);
      // c.append("circle");
      c.attr("id", "new_circle_ship_5")
        // .attr("class", "new_circle_ship_rings blink_ship")
        .attr("r", "21")
        .attr("cx", x)
        .attr("cy", y);
      // c.append("circle");
      c.attr("id", "new_circle_ship_6")
        // .attr("class", "new_circle_ship_rings blink_ship")
        .attr("r", "25")
        .attr("cx", x)
        .attr("cy", y);
    };
    const plotShipOnHover = (x, y) => {
      //render ship while hovering on panel
      let c = d3.select("#foreground").append("g").attr("id", "new_ship");
      c.append("circle")
    ...
    const drawFunctionTable = () => {
      ...
      d3.select(".functionality_table")
        ...
        .on("mouseover", plotShipOnHover)
        .on("mouseout", removeShip)
        .attr("id", "myRange")
        .attr("type", "range")
        .attr("min", 0)
        .attr("max", max);
      ...
      document
        ...
        .addEventListener("mousemove", function (e) {
          ...
          const change_date = (x) => {
            ...
            // if (nextt >= 0 && nextt <= 5) {
            //   nextslot = "00";
            // } else if (nextt >= 6 && nextt <= 11) {
            //   nextslot = "06";
            // } else if (nextt >= 12 && nextt <= 17) {
            //   nextslot = "12";
            // } else {
            //   nextslot = "18";
            // }
          ...
          var tmp2 = document.getElementById("myRange").value;

          if (!flag) {
            for (let i = 0; i < velocityarr.length; i++) {
              velocityarr[i] = parseFloat(velocityarr[i]) * parseFloat(3.6);
              sum = parseFloat(sum) + parseFloat(velocityarr[i]);
            }
            flag = 1;
          }
          avg = parseFloat(sum) / parseFloat(velocityarr.length);
          let tm = parseFloat(tmp2) / parseFloat(avg);
          ...
          reader.onload = function (e) {
            ...
            for (let i = 0; i < loadedPointsCSV.length; i++) {
              d3.select("#foreground")
                ...
                .attr("d", path);
              // drawLocationMark(loadedPointsCSV[i], [])
            }
            ...
            console.log("[Upload Coor arr]", coorarr);
    ...
    if (activeLocation.point && activeLocation.coord) {
      drawLocationMark(activeLocation.coord, activeLocation.point);
    }
    ...
    dispatch.listenTo(inputController, {
      moveStart: function () {
        ...
        rendererAgent.trigger("start"); // For animation overlay
      },
      ...
      moveEnd: function () {
        ...
        d3.select("#dot1").style("opacity", "1");
  ...
  function createMask(globe) {
    // console.log("Create Mask Function")
  ...
  function animate(globe, field, grids) {
    ...
    // if (configuration.attributes.level === "waves")
    //   g.lineWidth = 5.0;
  ...
  function showDate(grids) {
    var date = new Date();
    if (configuration.attributes.level === "currents") {
      date = new Date();
      date.setHours(t);
      date.setDate(d);
      date.setMonth(m - 1);
    } else {
      date = new Date(validityDate(grids));
    }
   ...
   */
  function createUnitToggle(id, product, type) {
  ...
  function init() {
    ...
    // Modify overlay menu depending on what mode we're in.
    ...
    const change_date = (x) => {
      ...
      if (Math.abs(x) === 24) {
        ...
        datetime.setHours(datetime.getHours() + x - 24);
    ...
    d3.select("#calender-input").on("change", function () {
      console.log(document.getElementById("calender-input").value);
      if (document.getElementById("calender-input").value) {
        ...
        datetime = new Date(date_from_calender + " " + time);
