          ...
          move: function (mouse, scale) {
            if (mouse) {
              ...
              if (current_proj == "o") {
                ...
                if (!d3.select("#weather_picker_btn").classed("inactive") && !d3.select("#dangerdot-tsunami").classed("active")
                && !d3.select("#dangerdot-sandstorm").classed("active") && !d3.select("#dangerdot-storm").classed("active") && !d3.select("#dangerdot-gale").classed("active")&& !d3.select("#dangerdot-hurricane").classed("active")
                )
                {
                  projection.rotate([xd * sensitivity, 0, projection.rotate()[2]]);   // Y-axis drag

                  let correction = window.innerHeight / 2 + scale * 1.08 - 260;
 
                  projection.translate([window.innerWidth / 2 , correction + d3.event.translate[1]]);      // correction for initial drag (initially)
                }                 
              }
            } 
            

            // Increased the zoom-in / zoom-out scale

            if(prev !== null){

              if(prev - scale < 0)
              {
                if(projectionScale + 12*(scale - prev) <= 6000)
                {
                  projectionScale += 12*(scale - prev)
              ...
              {
                if(projectionScale + 35*(scale - prev) >= 220)
                {
                  projectionScale += 35*(scale - prev)
                  projection.scale(projectionScale)
                }
              }
            }
