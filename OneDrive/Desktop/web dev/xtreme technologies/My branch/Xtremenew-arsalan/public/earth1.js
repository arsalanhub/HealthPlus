...
function windDataGenerator(prefilename){
    if(windObject[prefilename] !== undefined){
      return;
    }
    ...
    var promise = new Promise(function(resolve, reject) {
      ...
      xhr.open(
        "GET",
        `./wind/${m}_${data_day}_${data_slot}_${hours_to_be_added}_wind.gz`,
        true
      );
      xhr.onload = function() {
        if (xhr.status == 200) {
...
function waveDataGenerator(prefilename){
    ...
    if(waveObject[prefilename] !== undefined || waveGzipObject[prefilename] !== undefined){
      return false;
    }
    ...
    var promise = new Promise(function(resolve, reject) {
      ...
      xhr.open(
        "GET",
        `./wave/${m}_${data_day}_${data_slot}_${hours_to_be_added}_wave.gz`,
        true
      );
      xhr.onload = function() {
        if (xhr.status == 200) {
...
function dataGenerator() {
  ...
  let number_of_file = 3;

  for(let i=0; i<100 && number_of_file > 0; i++)
  {
    ...
    if(Number(hours_to_be_added) > 90)
      break;

    windDataGenerator(prefilename);
...
var products = (function () {
  ...
  function gfs1p0degPath(type, year, month, day, slot) {
    ...
    if (type === "wind") {
      ...
      if(waveObject[filename] !== undefined){      // waveset of extracted files (readable form data)
        found = true;
      }

      if(!found && waveGzipObject[filename] !== undefined){      // waveGzipset of xhr response (binary data)
      ...
      if (!found) {
        ...
        xhr.open(
          "GET",
          `./wave/${month}_${data_day}_${data_slot}_${hours_to_be_added}_wave.gz`,
          false
        );
        xhr.overrideMimeType("text/plain; charset=x-user-defined");
        xhr.send();
